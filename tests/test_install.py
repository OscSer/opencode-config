...
class TestConfigInstaller(unittest.TestCase):
    ...
    def test_resolve_source_not_found(self, mock_exists, mock_print):
        ...
        mock_print.assert_called_with("Warning: nonexistent.json not found in repository, skipping...")
    ...
    def test_get_target_dir(self):
        ...
        self.assertEqual(self.installer.get_target_dir('codex'), self.codex_dir)
    ...
    def test_create_symlink_success(self, mock_mkdir, mock_unlink, mock_is_symlink, mock_exists, mock_symlink):
    ...
    @patch('pathlib.Path.unlink')
    @patch('pathlib.Path.mkdir')
    def test_create_symlink_failure(self, mock_mkdir, mock_unlink, mock_is_symlink, mock_exists, mock_symlink):
    ...
    @patch('pathlib.Path.mkdir')
    def test_install_agent_success(self, mock_mkdir, mock_special_actions, mock_symlink, mock_resolve):
    ...
    def test_install_all_success(self, mock_install_agent):
        ...
        self.assertEqual(mock_install_agent.call_count, 3)
    ...
    @patch('builtins.open', new_callable=mock_open, read_data='{"mcpServers": {"test": "server"}}')
    ...
    def test_update_claude_mcp_config_success(self, mock_tempfile, mock_mkdir, mock_fsync, mock_replace, mock_json_dump, mock_resolve, mock_exists, mock_file):
    ...
    @patch('pathlib.Path.exists')
    @patch('install.ConfigInstaller.resolve_source')
    def test_update_claude_mcp_config_missing_mcp_servers(self, mock_resolve, mock_exists, mock_file):
    ...
    @patch('pathlib.Path.exists')
    @patch('install.ConfigInstaller.resolve_source')
    def test_update_claude_mcp_config_json_decode_error(self, mock_resolve, mock_exists, mock_file):
    ...
    def test_update_claude_mcp_config_source_not_found(self, mock_print, mock_resolve):
        ...
        mock_print.assert_called_with("Warning: claude/.mcp.json not found, skipping MCP configuration...")
    ...
    def test_validate_source_path_not_exists(self, mock_exists, mock_print):
        ...
        mock_print.assert_called_with("Warning: missing.json not found in repository, skipping...")

    @patch('builtins.print')
    @patch('pathlib.Path.exists', return_value=True)
    @patch('pathlib.Path.is_file', return_value=False)
    def test_validate_source_path_file_but_directory(self, mock_is_file, mock_exists, mock_print):
    ...
    def test_validate_source_path_directory_but_file(self, mock_is_dir, mock_exists, mock_print):
        """Test validation when expecting directory but path is file."""
        result = self.installer._validate_source_path('test_dir', is_file=False)
        self.assertIsNone(result)
        mock_print.assert_called_with("Warning: test_dir is not a directory, skipping...")
    ...
    @patch('shutil.rmtree')
    @patch('pathlib.Path.mkdir')
    def test_create_symlink_remove_existing_directory(self, mock_mkdir, mock_rmtree, mock_is_dir, mock_is_symlink, mock_exists, mock_symlink):
    ...
    @patch('pathlib.Path.unlink')
    @patch('pathlib.Path.mkdir')
    def test_create_symlink_remove_existing_file(self, mock_mkdir, mock_unlink, mock_is_dir, mock_is_symlink, mock_exists, mock_symlink):
    ...
    def test_run_special_actions_unknown_action(self, mock_print):
        ...
        mock_print.assert_called_with("Warning: Unknown special action 'unknown_action' for claude")
    ...
    @patch('install.ConfigInstaller._run_special_actions')
    @patch('pathlib.Path.mkdir')
    def test_install_agent_symlink_failure(self, mock_mkdir, mock_special_actions, mock_symlink, mock_resolve):
    ...
    @patch('install.ConfigInstaller._run_special_actions')
    @patch('pathlib.Path.mkdir')
    def test_install_agent_source_not_found(self, mock_mkdir, mock_special_actions, mock_resolve):
    ...
    @patch('pathlib.Path.mkdir')
    @patch('builtins.print')
    def test_install_agent_multiple_assets(self, mock_print, mock_mkdir, mock_special_actions, mock_symlink, mock_resolve):
