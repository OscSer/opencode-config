---
name: code-review
description: Perform systematic code reviews, focusing on bugs, security issues, and changed lines only
---
...
Report when changed code duplicates other changed code in the changed code:
...
| **Utility function replacement**      | New code matches existing util (observable in changes)    | CONCERN  | Use existing utility instead             |
| **Similar logic with minor variance** | Same control flow + variable names differ in changed code | CONCERN  | Extract shared logic, parameterize diffs |
...
1. **Use explicit type annotations** in the changed code (preferred)
2. **Use pattern evidence** in the same file (null checks, guards, return statements visible in changed code)
